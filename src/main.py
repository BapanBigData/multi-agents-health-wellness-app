...
from src.agent.graph import app as travel_graph
...
async def process_trip(job_id: str, message: str, context: Optional[Dict[str, Any]] = None):
    try:
...
async def start_trip(query: TravelQuery, bg: BackgroundTasks):
    ...
    bg.add_task(process_trip, job_id, query.message, query.context)
    print(query.message)
    print("\n")
    print(query.context)
    print("\n")
